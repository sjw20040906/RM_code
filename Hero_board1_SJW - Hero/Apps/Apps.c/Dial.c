...
void Dial_Processing_2(void)
{
  ...
  // if (Dial_Data.Shoot_Mode == Single_Shoot && Dial_Data.Dial_Switch == Dial_On && !is_dial_rotating)
  // {
  ...
  static bool is_dial_rotating = false;    // 是否正在转动
  static float target_rotate_angle = 0.0f; // 目标转动角度（当前角度+60度）

  // 主逻辑：整合判断条件，用嵌套结构处理不同状态
  if (Dial_Data.Shoot_Mode == Single_Shoot) // 先判断是否为单发模式
  {
    if (!is_dial_rotating) // 未处于转动状态时，检查启动条件
    {
      // 启动条件：拨盘开关打开，且未在转动中
      if (Dial_Data.Dial_Switch == Dial_On)
      {
        // 初始化转动参数：目标角度为当前角度+60度，标记为正在转动
        target_rotate_angle = M3508_Array[Dial_Wheel].totalAngle + 60.0f;
        is_dial_rotating = true;
      }
    }
    else // 处于转动状态时，执行转动控制
    {
      ...
      // 检查是否到达目标角度（允许±2度误差，避免抖动）
      if (fabs(M3508_Array[Dial_Wheel].totalAngle - target_rotate_angle) <= 2.0f)
      {
        // 转动完成：停止电机、关闭拨盘开关、重置状态
        M3508_Array[Dial_Wheel].outCurrent = 0; // 停止输出
        Dial_Data.Dial_Switch = Dial_Off;       // 关闭拨盘开关
        is_dial_rotating = false;               // 重置转动标志
