...
void Fric_Processing()
{
  ...
  {
    // 如果速度为0，停止所有电机
    M3508_Array[Fric_Left].targetSpeed = 0;
    M3508_Array[Fric_Right].targetSpeed = 0;
  }

  /************PID计算************/
  M3508_Array[Fric_Left].outCurrent = PID_Model4_Update(&M3508_FricL_Pid, &fuzzy_pid_shoot_L, M3508_Array[Fric_Left].targetSpeed, M3508_Array[Fric_Left].realSpeed);
  M3508_Array[Fric_Right].outCurrent = PID_Model4_Update(&M3508_FricR_Pid, &fuzzy_pid_shoot_R, M3508_Array[Fric_Right].targetSpeed, M3508_Array[Fric_Right].realSpeed);
}
...
void Fric_Judge_ReadyOrNot()
{
  ...
  temp[0] = M3508_Array[Fric_Left].targetSpeed - M3508_Array[Fric_Left].realSpeed;
  temp[1] = M3508_Array[Fric_Right].targetSpeed - M3508_Array[Fric_Right].realSpeed;

  // arm_abs_q15(temp, abs_differ, 3);
  abs_differ[0] = (temp[0] > 0) ? temp[0] : -temp[0];
  abs_differ[1] = (temp[1] > 0) ? temp[1] : -temp[1];

  temp[0] = M3508_Array[Fric_Left].targetSpeed * 0.05f;
  temp[1] = M3508_Array[Fric_Right].targetSpeed * 0.05f;
