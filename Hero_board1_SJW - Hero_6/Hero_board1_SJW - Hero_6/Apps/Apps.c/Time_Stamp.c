...
uint32_t GetTimeStamp(RTC_TimeTypeDef *Time, RTC_DateTypeDef *Date)
{
	...
	TimeCount = (uint32_t)Time->Seconds + (uint32_t)Time->Minutes * 60 + (uint32_t)Time->Hours * 3600 + (uint32_t)Date->Year * 31536000 + ((uint32_t)Date->Date - 1) * 86400;
	switch (Date->Month)
	{
	case 0x01:
		break;
